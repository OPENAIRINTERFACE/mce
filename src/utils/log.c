...
void log_set_config(const log_config_t * const config)
{
  if (config) {
    ...
    if ((MAX_LOG_LEVEL > config->m2ap_log_level) && (MIN_LOG_LEVEL <= config->m2ap_log_level))       g_oai_log.log_level[LOG_M2AP]     = config->m2ap_log_level;
    if ((MAX_LOG_LEVEL > config->m3ap_log_level) && (MIN_LOG_LEVEL <= config->m3ap_log_level))       g_oai_log.log_level[LOG_M3AP]     = config->m3ap_log_level;
    if ((MAX_LOG_LEVEL > config->mce_app_log_level) && (MIN_LOG_LEVEL <= config->mce_app_log_level)) g_oai_log.log_level[LOG_MCE_APP]  = config->mce_app_log_level;
    if ((MAX_LOG_LEVEL > config->sm_log_level) && (MIN_LOG_LEVEL <= config->sm_log_level))           g_oai_log.log_level[LOG_SM]       = config->sm_log_level;
  ...
  const int max_threadsP)
{
  ...
  snprintf (&g_oai_log.log_proto2str[LOG_M2AP][0], LOG_MAX_PROTO_NAME_LENGTH, "M2AP");
  snprintf (&g_oai_log.log_proto2str[LOG_M3AP][0], LOG_MAX_PROTO_NAME_LENGTH, "M3AP");
  snprintf (&g_oai_log.log_proto2str[LOG_MCE_APP][0], LOG_MAX_PROTO_NAME_LENGTH, "MCE-APP");
  snprintf (&g_oai_log.log_proto2str[LOG_SM][0], LOG_MAX_PROTO_NAME_LENGTH, "SM");
