 ...
 * The MBMS Session Start Request message shall be sent on the Sm/Sn interface by the MBMS GW to the MCE/SGSN
 * as specified in 3GPP TS 23.246 [37] and 3GPP TS 23.007 [13].
 */
typedef struct itti_sm_mbms_session_start_request_s {
  teid_t                             teid;                  ///< SM-MCE Tunnel Endpoint Identifier
 ...
 * on the Sm/Sn interface by the MCE/SGSN to the MBMS GW.
 */
typedef struct itti_sm_mbms_session_start_response_s {
  teid_t                             teid;                  ///< SM-MBMS-GW Tunnel Endpoint Identifier
  fteid_t                            sm_mce_teid;          ///< SM-MCE Tunnel Endpoint Identifier

  // here fields listed in 3GPP TS 29.274
  gtpv2c_cause_t               		 cause;               	///< If the MCE could successfully establish the MBMS service and the bearers.
 ...
 * MCE/SGSN as specified in 3GPP TS 23.246 [37] and 3GPP TS 23.007 [13].
 */
 ...
 * message on the Sm/Sn interface by the MCE/SGSN to the MBMS GW.
 */
typedef struct itti_sm_mbms_session_update_response_s {
  teid_t                             teid;                ///< SM-MBMS-GW Tunnel Endpoint Identifier
  teid_t                             mms_sm_teid;         ///< SM-MCE Tunnel Endpoint Identifier

  // here fields listed in 3GPP TS 29.274
  gtpv2c_cause_t               		 cause;               ///< If the MCE could successfully establish the UE context and the beaers.
 ...
 *                      Evolved General Packet Radio Service (GPRS);
 *                      Tunnelling Protocol for Control plane (GTPv2-C); Stage 3
 * The MBMS Session Stop Request message shall be sent on the Sm/Sn interface by the MBMS GW to the MCE/SGSN
 * as specified in 3GPP TS 23.246 [37] and 3GPP TS 23.007 [13].
 */
 ...
 * interface by the MCE/SGSN to the MBMS GW.
 */
typedef struct itti_sm_mbms_session_stop_response_s {
  ...
  // todo: Indication : This IE shall be included if any of the flags are set to 1. SGW Change Indication:   - This flag shall be set to 1 if the target MCE/SGSN   has selected a new SGW.
...
typedef struct itti_sm_remove_tunnel_s {
  /** Local Tunnel TEID. */
  teid_t                  local_teid;                ///< Sm MCE local Tunnel Endpoint Identifier
